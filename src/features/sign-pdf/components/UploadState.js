...
import { PenTool, Upload, ShieldCheck, AlertCircle, Lock, Unlock } from "lucide-react";
...
export default function UploadState({ onFileSelected, isLoading = false, error = null }) {
  ...
  const isPasswordError = error && (
    ...
    error.toLowerCase().includes("encrypted") ||
  ...
  return (
    <div className="max-w-3xl mx-auto text-center space-y-6">
        ...
        <div className="space-y-4 flex flex-col items-center">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-sm font-semibold text-slate-500 max-w-md mx-auto">
              Add your handwritten signature, typed calligraphy, or uploaded image signature directly to any PDF page. 100% private in your browser.
            </p>
          </div>
          ...
          <p className="text-xs font-bold text-slate-400">
            Supports contracts, agreements, invoices, forms & legal PDFs
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-2 text-xs font-extrabold text-slate-500 bg-slate-50 border border-slate-200/80 rounded-2xl py-3 px-4 max-w-md mx-auto">
        ...
        <span>100% Client-Side Privacy — Your PDF & signature never leave your device</span>
      </div>
    </div>
  );
}
