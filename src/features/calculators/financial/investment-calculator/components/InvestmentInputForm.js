...
import { CURRENCIES } from "@/features/calculators/financial/emi-calculator/utils/currency"; // Reusing from EMI calculator
...
export default function InvestmentInputForm({ 
  mode, 
  data, setData, 
  dataB, setDataB, 
  ...
  // Common fields
  const principalField = (isB = false) => renderInput("Initial Investment", "principal", isB ? dataB.principal : data.principal, isB, 'currency', true);
  ...
  const returnField = (isB = false) => renderInput("Expected Return Rate", "annualRate", isB ? dataB.annualRate : data.annualRate, isB, 'percentage', '%');
  const yearsField = (isB = false) => renderInput("Investment Period", "years", isB ? dataB.years : data.years, isB, 'number', 'Years');
  ...
  const futureValueField = () => renderInput("Future Value", "futureValue", data.futureValue, false, 'currency', true);
  const inflationRateField = () => renderInput("Expected Inflation Rate", "inflationRate", data.inflationRate, false, 'percentage', '%');

  if (mode === 'comparison') {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <FormSection title="Investment Scenario A" icon={<Icons.TrendingUp size={20} />} controls={renderCurrencySelector()}>
        ...
        <FormSection title="Investment Scenario B" icon={<Icons.ArrowRightLeft size={20} />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {principalField(true)}
